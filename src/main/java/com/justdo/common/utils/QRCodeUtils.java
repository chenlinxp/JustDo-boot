...
public class QRCodeUtils {
	...
	private static BufferedImage createImage(String content, String imgPath, int size, boolean needCompress) throws Exception {
	...
	public static String encodeZxingCode(String content, String path, Integer size, String logoPath)  {
		...
		try {
			...
			//生成二维码存放文件
			if(path.endsWith("/")){
				filepath = path+size.toString()+".jpg";
			}
			else{
				filepath =  path + "/"+size.toString()+".jpg";
			}
	...
	public static boolean zxingCodeCreate(String content, String path, Integer size, String logoPath) {
		try {
			...
			Random random = new Random();
			//生成二维码存放文件
			File file = new File(path+size.toString()+".jpg");
			if (!file.exists()) {
				file.mkdirs();
			}
			ImageIO.write(image, imageType, file);
	...
	public static String encode(String content, String destPath, boolean needCompress, int size) throws Exception {
		String file = encode(content, (String)null, destPath, needCompress, size);
		return file;
	}

	public static String encode(String content, String imgPath, String destPath, boolean needCompress, int size) throws Exception {
		BufferedImage image = createImage(content, imgPath, size, needCompress);
		mkdirs(destPath);
